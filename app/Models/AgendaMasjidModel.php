...
/**
 * Model untuk tabel tbl_agenda_masjid
 * Menyimpan agenda kegiatan Ramadhan mandiri per masjid/mushola
 */
class AgendaMasjidModel extends Model
{
    protected $table            = 'tbl_agenda_masjid';
    ...
    protected $allowedFields    = [
        'id_masjid_mushola', 'tanggal', 'waktu_mulai', 'waktu_selesai',
    ...
    /**
     * Ambil agenda masjid dengan join data personil (mubaligh)
     */
    public function getAgendaDenganDetail(int $idMasjid, array $filters = []): array
    {
        $builder = $this->db->table('tbl_agenda_masjid a')
            ...
            ->where('a.id_masjid_mushola', $idMasjid);
     ...
     * Digunakan untuk tampilan berdampingan dengan agenda mandiri
     */
     ...
     */
    public function getAgendaMendatang(int $idMasjid, int $hariKedepan = 7): array
    {
        $tglAwal   = date('Y-m-d');
        $tglAkhir  = date('Y-m-d', strtotime("+{$hariKedepan} days"));

        return $this->db->table('tbl_agenda_masjid a')
    ...
    /**
     * Ambil range tahun yang tersedia di agenda masjid (untuk filter dropdown)
     */
    public function getTahunTersedia(int $idMasjid): array
    {
        $rows = $this->db->table('tbl_agenda_masjid')
        ...
        // Selalu sertakan tahun sekarang dan besok meskipun belum ada data
