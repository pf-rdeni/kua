...
<div class="row">
    <div class="col-12">
        ...
        <?php
            ...
            $totalUserAda  = count($userPerEntitas);
            $totalBelumAda = $totalEntitas - $totalUserAda;
        ...
        <div class="row mb-3">
            <div class="col-md-4">
                <div class="info-box">
                    <span class="info-box-icon bg-primary"><i class="fas <?= $icon ?>"></i></span>
                    <div class="info-box-content">
                        <span class="info-box-text">Total <?= esc($cfg['title']) ?></span>
                        <span class="info-box-number"><?= $totalEntitas ?></span>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">
                    <i class="fas fa-users-cog mr-2"></i>Daftar User Akun - <?= esc($cfg['title']) ?>
                </h3>
            </div>
            <div class="card-body p-0">
                ...
                <div class="callout callout-info mx-3 mt-3">
                    <small>
                        ...
                        Klik tombol <strong><i class="fas fa-user-cog"></i> Kelola User</strong> untuk mengatur user operator / akun login,
                        termasuk reset password dan nonaktifkan akun.
                        Password default untuk user baru: <code><?= esc($defaultPw) ?></code>
                    </small>
                </div>

                <div class="table-responsive">
                    <table class="table table-bordered table-hover table-sm mb-0" id="tabelUserEntitas">
                        <thead class="thead-dark">
                            <tr>
                                <th style="width: 40px;">No</th>
                                <th>Nama <?= esc($cfg['title']) ?></th>
                                <th>User Operator/Akun</th>
                                <th>Status User</th>
                        ...
                        <tbody>
                            ...
                            <?php
                                $eId     = $ent[$cfg['pk']];
                                $users   = $userPerEntitas[$eId] ?? [];
                                $adaUser = !empty($users);
                            ?>
                            <tr class="<?= !$adaUser ? 'table-warning' : '' ?>">
                                <td><?= $no++ ?></td>
                                <td>
                                    <strong><?= esc($ent[$cfg['name_field']]) ?></strong>
                                    <?php if ($entitasType === 'mubaligh' && !empty($ent['nia'])): ?>
                                        <br><small class="text-muted">NIA: <?= esc($ent['nia']) ?></small>
                                    <?php elseif (!empty($ent['alamat'])): ?>
                                        <br><small class="text-muted"><?= esc(substr($ent['alamat'], 0, 40)) ?></small>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <?php if ($adaUser): ?>
                                ...
                                <td>
                                    <a href="<?= base_url($cfg['url_users'] . '/' . $eId . '/users') ?>"
                                       class="btn btn-<?= $adaUser ? 'primary' : 'warning' ?> btn-sm"
                                       title="<?= $adaUser ? 'Kelola User' : 'Buat User' ?>">
