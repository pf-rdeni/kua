...
<div class="row">
    <div class="col-md-8 mx-auto">
        <div class="card card-primary">
            ...
            <form action="<?= base_url('admin/setting-berkas/update/' . $setting['id']) ?>" method="post">
                <?= csrf_field() ?>
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nama_berkas" id="nama_berkas" class="form-control" value="<?= old('nama_berkas', $setting['nama_berkas']) ?>" placeholder="Misal: KTP, KK, Buku Rekening" required>
                    ...
                    <div class="form-group">
                        ...
                        <?php 
                            $selectedEntitas = explode(',', old('entitas_type', $setting['entitas_type'])); 
                        ?>
                        <select name="entitas_type[]" class="form-control select2" multiple="multiple" data-placeholder="Pilih Entitas" style="width: 100%;" required>
                            <?php
                                $options = [
                                    ...
                                    'fardu_kifayah' => 'Fardu Kifayah',
                                    'penggali_kubur' => 'Penggali Kubur',
                                    'majelis_taklim' => 'Majelis Taklim',
                                    'masjid_mushola' => 'Masjid Mushola',
                                    'tpq_mdta' => 'TPQ / MDTA'
                                ];
                                foreach($options as $val => $label):
                                    $selected = in_array($val, $selectedEntitas) ? 'selected' : '';
                            ?>
                    ...
                    <div class="row" id="custom_rasio_inputs" style="display: none;">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="number" step="any" name="aspect_ratio_width" id="aspect_ratio_width" class="form-control" value="<?= old('aspect_ratio_width', $setting['aspect_ratio_width']) ?>" placeholder="Misal: 85.6">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="aspect_ratio_height">Aspect Ratio (Height/Tinggi)</label>
                                <input type="number" step="any" name="aspect_ratio_height" id="aspect_ratio_height" class="form-control" value="<?= old('aspect_ratio_height', $setting['aspect_ratio_height']) ?>" placeholder="Misal: 53.98">
                            </div>
                        </div>
                        <div class="col-12">
                    ...
                    <div class="form-group border-top pt-3 mt-3">
                        ...
                        <select name="is_rekening" id="is_rekening" class="form-control">
                            <option value="0" <?= old('is_rekening', $setting['is_rekening'] ?? 0) == '0' ? 'selected' : '' ?>>Berkas Biasa (KTP, Ijazah, dll)</option>
                            <option value="1" <?= old('is_rekening', $setting['is_rekening'] ?? 0) == '1' ? 'selected' : '' ?>>Buku Tabungan / Rekening Bank</option>
                        </select>
                    ...
                    <div class="form-group" id="rekening_digit_group" style="display: <?= old('is_rekening', $setting['is_rekening'] ?? 0) == '1' ? 'block' : 'none' ?>;">
                        <label for="rekening_digit">Validasi Panjang Nomor Rekening (Digit)</label>
                        <input type="number" name="rekening_digit" id="rekening_digit" class="form-control" value="<?= old('rekening_digit', $setting['rekening_digit'] ?? '') ?>" placeholder="Misal: 10 atau 15">
                    ...
                    <div class="form-group border-top pt-3 mt-3">
                        ...
                        <select name="cetak_tipe" id="cetak_tipe" class="form-control">
                            <option value="gabung" <?= old('cetak_tipe', $setting['cetak_tipe'] ?? 'gabung') == 'gabung' ? 'selected' : '' ?>>Gabungkan Sebaris / Kolase (KTP, Buku Rekening, Foto)</option>
                            <option value="full_page" <?= old('cetak_tipe', $setting['cetak_tipe'] ?? 'gabung') == 'full_page' ? 'selected' : '' ?>>Satu Halaman Penuh Berdiri Sendiri (KK, Ijazah, SK)</option>
                        </select>
                    ...
                    <div class="form-group" id="cetak_lebar_group">
                        ...
                        <input type="number" min="10" max="500" name="cetak_lebar" id="cetak_lebar" class="form-control" value="<?= old('cetak_lebar', $setting['cetak_lebar'] ?? 100) ?>">
                    ...
                    <div class="form-group border-top pt-3 mt-3">
                        ...
                        <select name="status_aktif" id="status_aktif" class="form-control">
                            <option value="1" <?= old('status_aktif', $setting['status_aktif']) == '1' ? 'selected' : '' ?>>Aktif</option>
                            <option value="0" <?= old('status_aktif', $setting['status_aktif']) == '0' ? 'selected' : '' ?>>Nonaktif</option>
                        </select>
                    </div>
